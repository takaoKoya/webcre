...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    else if (isDevMode) router.push('/dashboard');
    else setDone(true);
    setLoading(false);
  };
  ...
  const handleDevSkip = () => router.push('/dashboard');
    ...
    <div className="min-h-screen bg-[#0d1117] flex flex-col items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white mt-6 mb-1">アカウント作成</h1>
          <p className="text-white/50 text-sm">無料で始められます。クレジットカード不要。</p>
        </div>
        ...
        <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.03]">
          {done ? (
            <div className="text-center py-4">
              ...
              <h2 className="text-white font-semibold mb-1">確認メールを送信しました</h2>
              <p className="text-white/50 text-sm">{email} に確認リンクを送りました。メールをご確認の上、アカウントを有効化してください。</p>
            </div>
          ) : (
            <>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-white/60 text-xs mb-1.5">お名前（任意）</label>
                ...
                <div>
                  <label className="block text-white/60 text-xs mb-1.5">パスワード（8文字以上）</label>
                  <div className="relative">
                ...
                >
                  {loading ? (
                    ...
                    <>無料アカウントを作成 <ArrowRight className="w-4 h-4" /></>
                  )}
                </button>
              </form>
              ...
              <p className="text-center text-white/30 text-xs mt-4">
                登録することで
                <Link href="#" className="text-purple-400 hover:underline mx-1">利用規約</Link>
                および
                <Link href="#" className="text-purple-400 hover:underline mx-1">プライバシーポリシー</Link>
                に同意したものとみなされます
              </p>
            </>
          )}
